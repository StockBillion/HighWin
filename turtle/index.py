#!/usr/bin/env python
#-*- coding: utf8 -*-
import argparse, numpy, math, datetime
...
def fit_line(dates, prices):
    _count = min(len(dates), len(prices))
    x1, y1, x2, y2, xy = 0, 0, 0, 0, 0
    dx2, dy2, dxy = 0, 0, 0
...
#     sd2, sp2, sdp = 0, 0, 0
#     for i in range(0, _count):
#         sd2 += (dates[i] - dmean) * (dates[i] - dmean)
#         sp2 += (prices[i] - pmean) * (prices[i] - pmean)
#         sdp += (prices[i] - pmean) * (dates[i] - dmean)

#     slope = sdp/sd2
#     intercept = pmean - slope*dmean
#     r_square = sdp*sdp/(sd2*sp2)
#     return slope, intercept, r_square
...
# def moving_average(n, prices, ma_indexs = {}):
#     mas = []
...
#         if m1 not in ma_indexs:
#             moving_average(m1, prices, ma_indexs)
#         hs1 = ma_indexs[m1]

#         if m2 not in ma_indexs:
#             moving_average(m2, prices, ma_indexs)
#         hs2 = ma_indexs[m2]
